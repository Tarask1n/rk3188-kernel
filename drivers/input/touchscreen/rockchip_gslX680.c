...
#define CONFIG_TCHIP_MACH_TR976Q
#endif
...
static void gsl_load_fw(struct i2c_client *client)
{
	...
	if(is_noid_version)
	{
		printk("[GSLX680] 3680B detected\n");
		ptr_fw = GSL3680B_FW;
		source_len = ARRAY_SIZE(GSL3680B_FW);
	}
	else
	{
		printk("[GSLX680] 3680A detected\n");
		ptr_fw = GSL3680A_FW;
		source_len = ARRAY_SIZE(GSL3680A_FW);
	}
#else
	printk("[GSLX680] 3680A not compatible detected\n");
	ptr_fw = GSLX680_FW;
	source_len = ARRAY_SIZE(GSLX680_FW);
#endif
#if defined(GSL_DEBUG)
	printk("[GSLX680] FW: 0x%08x 0x%08x 0x%08x 0x%08x\n", 
...
static void gslX680_ts_worker(struct work_struct *work)
{
	...
	if(tmp1 > 0 && tmp1 < 0xffffffff)
	{
		...
		gsl_ts_write(ts->client, 0x08, buf, 4);
	}
